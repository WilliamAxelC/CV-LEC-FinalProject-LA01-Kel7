...
experiment_name = "PersonDetection-HOG-SVM-CV-FinalProject"
mlflow.set_experiment(experiment_name)

# User-defined run name
run_name = "HOG-SVM-RandomResample-NoFeatureSelector-Augmentation"

# Set Positive Label for dataset labeling
positive_label = "Persona"
...
# Dataset paths
dataset_path = r"D:\AI stuff\CV-LEC-FinalProject\Datasets\Person detection.v6i-cropped-64x128"
face_dataset_path = r"D:\AI stuff\CV-LEC-FinalProject\Datasets\Face Detection.v9-Cropped-64x64-face"

# Load annotations CSV
annotations_path = os.path.join(dataset_path, "annotations.csv")
annotations = pd.read_csv(annotations_path)

# Load face dataset annotations
face_annotations_path = os.path.join(face_dataset_path, "annotations.csv")
face_annotations = pd.read_csv(face_annotations_path)


# Filter face dataset for "face" annotated images
face_annotations = face_annotations[face_annotations['file_class'] == 'face']

# Change all 'file_class' values to 'negative'
face_annotations['file_class'] = 'negative'
...
# Function to load and augment images using annotations.csv
def load_images_from_annotations(annotations, dataset_path, split, augmentations=None):
    data, labels = [], []
    ...
    for _, row in tqdm(subset.iterrows(), desc=f"Processing {split} data", total=len(subset)):
        ...
        try:
            ...
            print(f"Skipping {img_path}: File not found or cannot be opened.")
            continue
        ...
        if augmentations:
            try:
                ...
                print(f"Skipping augmentation for {img_path}: {e}")
                continue
...
train_data, train_labels = load_images_from_annotations(annotations, dataset_path, "train", augmentation)
val_data, val_labels = load_images_from_annotations(annotations, dataset_path, "valid", augmentation)
test_data, test_labels = load_images_from_annotations(annotations, dataset_path, "test")

# face_train_data, face_train_labels = load_images_from_annotations(face_annotations, face_dataset_path, "train", augmentation)
# face_val_data, face_val_labels = load_images_from_annotations(face_annotations, face_dataset_path, "valid", augmentation)
# face_test_data, face_test_labels = load_images_from_annotations(face_annotations, face_dataset_path, "test")

# train_data.extend(face_train_data)
# train_labels.extend(face_train_labels)
# val_data.extend(face_val_data)
# val_labels.extend(face_val_labels)
# test_data.extend(face_test_data)
# test_labels.extend(face_test_labels)
